...
import merelle.model.MerelleStageFactory;
...
public class MerelleStageModel extends GameStageModel {
    ...
    public MerellePawn[] getBlackPawns() {
        return blackPawns;
    }

    public MerellePawn[] getRedPawns() {
        return redPawns;
    }
    ...
    private void computePartyResult() {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if ((row >= 0) && (row <= 2) && (col >= 0) && (col <= 2)) {
                p = (MerellePawn) (board.getElement(row, col));
                if (p.getColor() == MerellePawn.PAWN_BLACK) {
                    nbBlack++;
